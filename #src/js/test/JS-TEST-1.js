...
const plusDelay = urlParams.get('plusDelay') ? parseInt(urlParams.get('plusDelay')) : 200
const maxDelay = urlParams.get('maxDelay') ? parseInt(urlParams.get('maxDelay')) : 1500
...
const observer = new IntersectionObserver((entries, observer) => {
	entries.forEach(entry => {
		if (entry.isIntersecting && !entry.target.classList.contains('animated')) {
			...
			const delay = maxDelay * k + tempDelay
